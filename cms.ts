...
/** Professor Training Office — courses, lessons, media, news. */
export async function assertStudio(userId: string) {
  ...
  const rows = await sql<{ user_id: string }>`
    ...
    limit 1
  `;
  if (!rows.length) throw new Error("The Training Office is closed to you.");
}
